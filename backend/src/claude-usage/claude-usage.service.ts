...
export class ClaudeUsageService {
  ...
  private get dbInstance(): Database.Database {
    return this.db.dbInstance;
  }

  getUsageSummary(filter: DateRangeFilter = {}) {
  ...
  getUsageTimeSeries(filter: DateRangeFilter = {}) {
    const { clause, params } = buildDateFilter(filter.dateFrom, filter.dateTo);
    const rows = this.dbInstance.prepare(`
      ...
      ORDER BY date ASC
    `).all(...params) as Array<{ date: string; inputTokens: number; outputTokens: number; cacheReadTokens: number; sessions: number }>;
  ...
  getToolBreakdown(filter: DateRangeFilter = {}) {
    const { clause, params } = buildDateFilter(filter.dateFrom, filter.dateTo);
    const rows = this.dbInstance.prepare(`
  ...
  getLanguageBreakdown(filter: DateRangeFilter = {}) {
    const { clause, params } = buildDateFilter(filter.dateFrom, filter.dateTo);
    const rows = this.dbInstance.prepare(`
  ...
  getModelBreakdown(filter: DateRangeFilter = {}) {
    const { clause, params } = buildDateFilter(filter.dateFrom, filter.dateTo);
    const rows = this.dbInstance.prepare(`
      SELECT
        model,
    ...
    `).all(...params) as Array<{ model: string; inputTokens: number; outputTokens: number; messageCount: number }>;
  ...
  getRecentSessions(limit = 10, filter: DateRangeFilter = {}) {
    const { clause, params } = buildDateFilter(filter.dateFrom, filter.dateTo);
    const rows = this.dbInstance.prepare(`
    ...
    const toolsBySession = new Map<string, Array<{ tool: string; count: number }>>();
    const toolRows = this.dbInstance.prepare(`
    ...
    const languagesBySession = new Map<string, Array<{ language: string; count: number }>>();
    const languageRows = this.dbInstance.prepare(`
  ...
  getFullResponse(filter: DateRangeFilter = {}) {
    return {
      ...
      timeSeries: this.getUsageTimeSeries(filter),
      toolBreakdown: this.getToolBreakdown(filter),
      languageBreakdown: this.getLanguageBreakdown(filter),
      modelBreakdown: this.getModelBreakdown(filter),
      recentSessions: this.getRecentSessions(10, filter),
